...
const GameHistory = ({ hotNumbers, coldNumbers, recentGames = [] }) => {
  ...
  const openExplorer = (txHash, network = 'creditcoin') => {
    if (txHash) {
      let explorerUrl;
      
      if (network === 'arbitrum-sepolia') {
        explorerUrl = getArbitrumSepoliaExplorerUrl(txHash);
      } else {
        explorerUrl = getCreditCoinTestnetExplorerUrl(txHash);
      }
  ...
  const NumberBall = ({ number, size = 32, opacity = 1, game = null }) => (
    <Tooltip 
      title={
        game?.vrfDetails?.transactionHash || game?.creditcoinTxHash ? (
          <Box>
            {game?.creditcoinTxHash && (
              <Typography variant="caption" display="block">
                CreditCoin TX: {game.creditcoinTxHash.slice(0, 10)}...
              </Typography>
            )}
    ...
    >
      <Box 
        sx={{ 
          ...
          cursor: (game?.vrfDetails?.transactionHash || game?.creditcoinTxHash) ? 'pointer' : 'default',
          '&:hover': {
            transform: (game?.vrfDetails?.transactionHash || game?.creditcoinTxHash) ? 'scale(1.1)' : 'scale(1.05)',
          }
        }}
        onClick={() => {
          if (game?.creditcoinTxHash) {
            openExplorer(game.creditcoinTxHash, 'creditcoin');
  ...
  return (
    ...
    >
      ...
      <Typography variant="body1" paragraph color="white">
        Track the results of previous spins and identify patterns. All results are verified using Pyth Entropy on Arbitrum Sepolia and logged on CreditCoin Testnet.
