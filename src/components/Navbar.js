"use client";
// Updated for Creditcoin migration - v2.1
...
import { CREDITCOIN_CONTRACTS, CREDITCOIN_NETWORKS } from '../config/contracts';
...
const TREASURY_CONFIG = {
  ADDRESS: CREDITCOIN_CONTRACTS[CREDITCOIN_NETWORKS.TESTNET].treasury
};
...
const MOCK_SEARCH_RESULTS = {
  ...
  tournaments: [
    { id: 'tournament1', name: 'High Roller Tournament', path: '/tournaments/high-roller', prize: '10,000 CTC' },
    { id: 'tournament2', name: 'Weekend Battle', path: '/tournaments/weekend-battle', prize: '5,000 CTC' },
  ],
  pages: [
...
export default function Navbar() {
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message: 'Your CTC balance has been updated',
      isRead: false,
      time: '2 min ago'
    },
    {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      console.log('🔍 Withdrawal amount (CTC):', balanceInCtc);
  ...
  const handleDeposit = async () => {
    ...
    const MAX_DEPOSIT = 10000; // Increased limit to 10000 CTC

    if (amount < MIN_DEPOSIT) {
      notification.error(`Minimum deposit amount is ${MIN_DEPOSIT} CTC`);
      return;
    }

    if (amount > MAX_DEPOSIT) {
      notification.error(`Maximum deposit amount is ${MAX_DEPOSIT} CTC`);
      return;
    }

    setIsDepositing(true);
    console.log('🚀 Starting deposit process for:', amount, 'CTC');
    try {
      ...
      // Check if user is on Creditcoin Testnet network
      const currentChainId = chainId;
      const expectedChainId = creditcoinTestnetConfig.id; // 102031

      console.log('🔍 Current chain ID:', currentChainId);
      console.log('🔍 Expected chain ID:', expectedChainId);

      if (currentChainId !== expectedChainId) {
        throw new Error(`Please switch to ${creditcoinTestnetConfig.name} network (Chain ID: ${expectedChainId}). Current chain: ${currentChainId}`);
      }

      console.log('✅ Already on correct network');

      // Casino treasury address from Creditcoin contracts config
      const TREASURY_ADDRESS = CREDITCOIN_CONTRACTS[CREDITCOIN_NETWORKS.TESTNET].treasury;
      ...
      notification.success(`Successfully deposited ${amount} CTC to casino treasury! TX: ${txHash.slice(0, 10)}...`);
        ...
        {isClient && showBalanceModal && createPortal(
          ...
          >
            ...
            >
              ...
              {/* Deposit Section */}
              <div className="mb-6">
                <h4 className="text-sm font-medium text-white mb-2">Deposit CTC to Casino Treasury</h4>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Transfer CTC from your wallet to house balance for gaming
                </p>
              ...
              {/* Withdraw Section */}
              <div className="mb-4">
                <h4 className="text-sm font-medium text-white mb-2">Withdraw CTC</h4>
                <button
                ...
                >
                  {isWithdrawing ? (
                    ...
                    parseFloat(userBalance || '0') > 0 ? 'Withdraw All CTC' : 'No Balance'
                  ) : 'Connect Wallet'}
