...
const RouletteWithHistory = () => {
  ...
  const [betConfig, setBetConfig] = useState({
    betType: 'straight',
    betValue: 7,
    betAmount: ethers.parseEther('0.01') // 0.01 CTC
  });
  ...
  const playGame = async () => {
    ...
    try {
      ...
      // NOTE: When integrated with CreditCoinGameLogger service, the creditcoinTxHash
      // would be captured from the logGameResult response and stored here:
      // const gameLogResult = await creditcoinGameLogger.logGameResult({...});
      // setGameState(prev => ({ ...prev, creditcoinTxHash: gameLogResult.creditcoinTxHash }));
        ...
        <div className="bg-white rounded-lg border p-6 mb-6">
          ...
          <div className={`p-4 rounded-lg text-center ${gameState.result.isWin ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'
            ...
            <div className="text-sm text-gray-600">
              Bet: {ethers.formatEther(betConfig.betAmount)} CTC →
              Payout: {ethers.formatEther(gameState.result.payoutResult.payout.toString())} CTC
            </div>
            <div className={`font-medium ${gameState.result.isWin ? 'text-green-600' : 'text-red-600'
              }`}>
              Profit: {gameState.result.isWin ? '+' : ''}{ethers.formatEther(gameState.result.payoutResult.profit.toString())} CTC
            </div>
          </div>
        </div>
      )}
      ...
      {gameState.vrfDetails && (
        <div className="bg-white rounded-lg border p-6 mb-6">
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 bg-purple-50 rounded-lg border border-purple-200">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xs font-semibold text-purple-700">ENTROPY</span>
              </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <a
                      ...
                      title="View entropy transaction"
                    >
            ...
            {/* CreditCoin Game Log Transaction */}
            {gameState.creditcoinTxHash && (
              <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-semibold text-green-700">CREDITCOIN TESTNET - GAME LOG</span>
                </div>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">Transaction Hash:</span>
                    <div className="flex items-center gap-2">
                      <code className="bg-white px-2 py-1 rounded text-xs">
                        {gameState.creditcoinTxHash.slice(0, 10)}...{gameState.creditcoinTxHash.slice(-8)}
                      </code>
                      <a
                        href={`https://creditcoin-testnet.blockscout.com/tx/${gameState.creditcoinTxHash}`}
                        target="_blank"
                        rel="noopener noreferrer"
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded text-xs text-blue-700">
            ...
            <p>
              Randomness from Pyth Entropy; game result logged on CreditCoin Testnet.
