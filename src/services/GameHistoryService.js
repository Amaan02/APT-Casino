 ...
 * CreditCoin Testnet for game logs; entropy verification supported
 */
export class GameHistoryService {
   ...
   * @returns {Promise<Object>} VRF/entropy details
   */
  async getVRFDetails(vrfRequestId) {
    ...
    try {
      ...
      const entropyExplorer = process.env.NEXT_PUBLIC_SEPOLIA_EXPLORER || 'https://sepolia.arbiscan.io';
      ...
      return {
        ...
        etherscanUrl: `${entropyExplorer}/tx/${vrf.transaction_hash}`,
        network: 'entropy',
        verifiable: true,
        verificationNote: 'Entropy via Pyth Entropy'
      };
  ...
  async getUserHistory(userAddress, options = {}) {
    ...
    try {
      ...
      // Build query with CreditCoin transaction hash
      let query = `
        SELECT 
          gr.id,
          gr.vrf_request_id,
      ...
      // Get explorer URLs (CreditCoin for game logs, entropy explorer for VRF)
      const creditcoinExplorer = process.env.NEXT_PUBLIC_CREDITCOIN_TESTNET_EXPLORER || 'https://creditcoin-testnet.blockscout.com';
      const entropyExplorer = process.env.NEXT_PUBLIC_SEPOLIA_EXPLORER || 'https://sepolia.arbiscan.io';
      ...
      const games = result.rows.map(row => {
        const game = {
          ...
          network: row.network || 'creditcoin-testnet'
        };
        ...
        // Add VRF details if available (entropy)
        if (includeVrfDetails && row.entropy_tx_hash) {
          game.vrfDetails = {
            ...
            etherscanUrl: `${entropyExplorer}/tx/${row.entropy_tx_hash}`,
            network: 'entropy',
            verifiable: true,
            verificationNote: 'Entropy via Pyth - click to verify'
          };
        }
