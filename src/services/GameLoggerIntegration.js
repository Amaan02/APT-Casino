 ...
 */
export async function logGameToCreditcoin({
  gameType,
  playerAddress,
  betAmount,
  result,
  ...
  try {
    ...
    console.log('✅ Game logged to Creditcoin:', {
      gameType,
      txHash: data.txHash,
    ...
    console.error('❌ Failed to log game to Creditcoin:', error);
...
/**
 * Get player's game history from Creditcoin
...
export async function getPlayerGameHistory(playerAddress, limit = 50, provider = null) {
  try {
    ...
    if (provider) {
      creditcoinGameLogger.setProviderAndSigner(provider, null);
    }

    const history = await creditcoinGameLogger.getGameHistory(playerAddress, limit);
...
export async function getPlayerGameCount(playerAddress, provider = null) {
  try {
    ...
    const count = await creditcoinGameLogger.getPlayerGameCount(playerAddress);
...
export async function getGameLoggerStats(provider = null) {
  try {
    ...
    const stats = await creditcoinGameLogger.getStats();
...
export function subscribeToGameResults(callback, provider = null) {
  try {
    ...
    return creditcoinGameLogger.onGameResultLogged(callback);
...
export function getTransactionExplorerUrl(txHash) {
  return creditcoinGameLogger.getTransactionUrl(txHash);
}
