...
/**
 * Get block explorer URL for transaction (CreditCoin Testnet by default)
 * @param {string} transactionHash - Transaction hash
 * @param {string} network - Network name (default: creditcoin-testnet)
 * @returns {string} Explorer URL
 */
export const getEtherscanUrl = (transactionHash, network = 'creditcoin-testnet') => {
  const baseUrls = {
  ...
  const baseUrl = baseUrls[network] || baseUrls['creditcoin-testnet'];
  return `${baseUrl}/tx/${transactionHash}`;
};

/**
 * Format CTC amount for display
 * @param {string|number} amount - Amount in wei
 * @param {number} decimals - Number of decimal places (default 18 for CTC)
