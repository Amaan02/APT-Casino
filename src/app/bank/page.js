...
// Assets for borrowing on Creditcoin Testnet only
const BORROW_ASSETS = {
  creditcoin_testnet: [
    {
      symbol: "CTC",
      name: "Creditcoin Testnet Network Coin",
      iconColor: "#000000",
...
const MOCK_TRANSACTIONS = [
  { type: 'deposit', token: 'CTC', amount: '120.5', date: new Date(Date.now() - 86400000 * 2), status: 'completed' },
  { type: 'borrow', token: 'CTC', amount: '0.3', date: new Date(Date.now() - 86400000), status: 'completed' },
  { type: 'swap', tokenFrom: 'CTC', tokenTo: 'USDC', amountFrom: '0.2', amountTo: '98.32', date: new Date(), status: 'completed' }
];

export default function Bank() {
  const [chainId, setChainId] = useState('creditcoin_testnet'); // Default to Creditcoin testnet
  ...
  useEffect(() => {
    ...
    if (isDev) {
      setChainId('creditcoin_testnet'); // Creditcoin testnet for development
      setAssets([
        {
          symbol: "CTC",
          name: "Creditcoin Testnet Network Coin",
    ...
    // Load Creditcoin testnet data
    const loadChainData = async () => {
      try {
        // Set to Creditcoin testnet
        setChainId('creditcoin_testnet');

        // Set mock lending market data for Creditcoin testnet
        setAssets([
          {
            symbol: "CTC",
            name: "Creditcoin Testnet Network Coin",
  ...
  // Get appropriate borrow assets for Creditcoin testnet
  const borrowAssets = BORROW_ASSETS.creditcoin_testnet;
        ...
        {showNetworkBanner && (
          <div className="bg-gradient-to-r from-red-magic/80 to-blue-magic/80 py-2 px-4 text-center relative mb-8 rounded-lg">
            <p className="text-white text-sm">
              Connected to Creditcoin Testnet.
            ...
            <>
              <div className="max-w-2xl mx-auto mb-12">
                <div className="bg-gradient-to-r p-[1px] from-red-magic to-blue-magic rounded-xl">
                  {/* Creditcoin Testnet Only - No Uniswap Integration */}
                  <div className="bg-gray-800 rounded-lg p-6 text-center">
                    <h3 className="text-xl font-semibold text-white mb-2">Creditcoin Testnet Only</h3>
                    <p className="text-gray-400">This application works exclusively with Creditcoin Testnet</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-[#250020] p-4 rounded-lg hover:bg-[#350030] transition-colors">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white/70 text-sm">CTC Price</span>
            ...
            <div className="bg-[#1A0015] rounded-xl p-6 h-full">
              ...
              <ul className="space-y-2 mb-4">
                <li className="flex justify-between">
                  <span className="text-white/60">CTC</span>
