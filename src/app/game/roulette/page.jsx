...
export default function GameRoulette() {
              ...
              <div className="bg-gradient-to-br from-red-900/20 to-red-800/5 rounded-xl p-4 border border-red-800/20 shadow-lg shadow-red-900/10">
                ...
                >
                  ...
                  <div className="flex flex-col items-center p-2 bg-black/20 rounded-lg min-w-0 w-full">
                    ...
                    <div className="text-white font-display text-sm md:text-base truncate w-full text-center" title={`${gameStatistics.totalVolume} CTC`}>
                      {gameStatistics.totalVolume} CTC
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center p-2 bg-black/20 rounded-lg min-w-0 w-full">
                    ...
                    <div className="text-white font-display text-sm md:text-base truncate w-full text-center" title={`${gameStatistics.maxWin} CTC`}>
                      {gameStatistics.maxWin} CTC
                    </div>
                  </div>
                </motion.div>
  ...
  // Function to fetch real CTC balance will be defined after useSelector
  ...
  // Function to fetch real CTC balance
  ...
  const lockBet = async () => {
    ...
    const currentBalance = parseFloat(userBalance || '0'); // Balance is already in CTC
    const totalBetAmount = total;

    if (currentBalance < totalBetAmount) {
      alert(`Insufficient balance. You have ${currentBalance.toFixed(5)} CTC but need ${totalBetAmount.toFixed(5)} CTC`);
      return;
    }
    ...
    try {
      ...
      if (originalBalance < totalBetAmount) {
        alert(`Insufficient balance. You have ${originalBalance.toFixed(5)} CTC but need ${totalBetAmount.toFixed(5)} CTC`);
      ...
      setTimeout(() => {
        ...
        pythEntropyService.generateRandom('ROULETTE', {
          ...
          // Log game result to CreditCoin Testnet (non-blocking)
          logGame({
            gameType: 'ROULETTE',
            ...
            if (txHash) {
              console.log('✅ Roulette game logged to Creditcoin:', getExplorerUrl(txHash));
              setBettingHistory(prev => {
                const updatedHistory = [...prev];
                if (updatedHistory.length > 0) {
                  updatedHistory[0] = { ...updatedHistory[0], creditcoinTxHash: txHash };
                }
                return updatedHistory;
              });
            }
          }).catch(error => {
            console.warn('⚠️ Failed to log Roulette game to Creditcoin:', error);
          });
        ...
        if (netResult > 0) {
          const winMessage = winningBets.length === 1
            ? `🎉 WINNER! ${winningBets[0].name} - You won ${(netResult - totalBetAmount).toFixed(5)} CTC!`
            : `🎉 MULTIPLE WINNERS! ${winningBets.length} bets won - Total: ${(netResult - totalBetAmount).toFixed(5)} CTC!`;

          setNotificationMessage(winMessage);
          setNotificationSeverity("success");
          setSnackbarMessage(winMessage);
        } else {
          setNotificationMessage(`💸 Number ${winningNumber} - You lost ${totalBetAmount.toFixed(5)} CTC!`);
          setNotificationSeverity("error");
          setSnackbarMessage(`💸 Number ${winningNumber} - You lost ${totalBetAmount.toFixed(5)} CTC!`);
        }
      ...
      const checkWithRetry = async (attempts = 3) => {
        if (window.ethereum && typeof window.ethereum.request === 'function') {
          ...
          try {
            ...
            // Support Creditcoin Testnet (0x18e8f)
            const isCorrectNetwork = chainId === "0x18e8f" || chainId === "102031";
  ...
  const switchNetwork = async () => {
    ...
    try {
      ...
      console.log("Attempting to switch to Creditcoin Testnet network");

      try {
        // Try Creditcoin Testnet first
        await window.ethereum.request({
          method: "wallet_switchEthereumChain",
          params: [{ chainId: "0x18e8f" }],
        });

        console.log("Successfully switched to Creditcoin Testnet");
        ...
        if (switchError.code === 4902) {
          try {
            console.log("Adding Creditcoin Testnet to wallet");
            await window.ethereum.request({
            ...
            try {
              ...
              console.log("Successfully switched to Creditcoin Testnet after adding");
              // Set network to correct and reload after short delay
              setCorrectNetwork(true);
              setTimeout(() => window.location.reload(), 1000);
              return;
            } catch (error) {
              console.error("Error switching to Creditcoin after adding:", error);
            }
          } catch (addError) {
            console.error("Failed to add Creditcoin Testnet:", addError);
            alert("Unable to switch to required network. Please try adding Creditcoin Testnet manually.");
          }
        } else {
          // Handle other errors
          console.error("Failed to switch network:", switchError);
          alert("Failed to switch network. Please try again or add Creditcoin Testnet manually.");
        }
      }
    } catch (error) {
      ...
      <div ref={contentRef} className="font-sans" style={{ backgroundColor: "#080005", minHeight: "100vh", overflowX: 'hidden', paddingTop: "30px" }}>
        ...
        >
          ...
          >
            ...
            >
              <FaCoins className="text-yellow-400" />
              Balance: {isConnected ? `${parseFloat(userBalance || '0').toFixed(5)} CTC` : 'Connect Wallet'}
            </Typography>
          </Box>
          ...
          >
            ...
            <Box sx={{
              ...
              <Typography color="white" sx={{ opacity: 0.8 }}>
                Current Bet Total: {total.toFixed(5)} CTC
              </Typography>
                ...
                {rollResult >= 0 ? (
                  ...
                  <Box sx={{ display: "flex", flexDirection: "column" }}>
                    <Button
                      ...
                      onClick={lockBet}
                    >
                      {total > 0 ? `Place Bet (${total.toFixed(5)} CTC)` : 'Place Bet (CTC)'}
                    </Button>
          ...
          <Grid container spacing={4} sx={{ mb: 7 }}>
            ...
            <Grid xs={12} md={6}>
              ...
              >
                ...
                >
                  European Roulette with a single zero and just no house edge - better odds than traditional casinos. Provably fair and powered by CreditCoin and Pyth Entropy on-chain randomness.
                </Typography>
        ...
        >
          ...
          >
            ...
            {notificationIndex === notificationSteps.RESULT_READY && (
              <Typography>
                {winnings > 0
                  ? `🎉 You won ${winnings.toFixed(4)} CTC!`
                  : winnings < 0
                    ? `💸 You lost ${Math.abs(winnings).toFixed(4)} CTC!`
                    : "🤝 Break even!"}
              </Typography>
            )}
          </MuiAlert>
