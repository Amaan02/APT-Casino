...
export default function GameControls({ 
  ...
  const handleBet = () => {
    ...
    if (betValue < 0.001) {
      alert("Minimum bet amount is 0.001 CTC");
      return;
    }
    
    if (betValue > currentBalance) {
      alert(`Insufficient balance! You have ${currentBalance.toFixed(5)} CTC but need ${betValue} CTC`);
      return;
    }
  ...
  const startAutoBetting = () => {
    ...
    if (totalBetAmount > currentBalance) {
      alert(`Insufficient balance for ${totalBets} bets of ${betAmount} CTC each. You need ${totalBetAmount.toFixed(3)} CTC but have ${currentBalance.toFixed(5)} CTC`);
      setIsAutoPlaying(false);
      return;
    }
  ...
  // Get current balance in CTC for display
  ...
  return (
    <div className="bg-[#1A0015] rounded-xl border border-[#333947] p-6">
      ...
      <div className="mb-6">
        ...
        <div className="mb-2">
          <span className="text-2xl font-bold text-white">{betAmount} CTC</span>
        </div>
      ...
      {gameMode === "auto" && isAutoPlaying ? (
        ...
        <div className="space-y-3">
          ...
          <div className="text-center p-3 bg-[#2A0025] rounded-lg border border-[#333947]">
            ...
            {isConnected ? (
              <div className="text-lg font-bold text-green-400">{getCurrentBalanceInMON()} CTC</div>
            ) : (
          ...
          {((gameMode === "auto" && !hasSufficientBalanceForAutoBet()) || (!gameMode === "auto" && !hasSufficientBalance())) && parseFloat(betAmount) > 0 && (
            <div className="text-center text-red-400 text-sm">
              {gameMode === "auto" 
                ? `Insufficient balance CTC each` 
                : `Insufficient balance CTC bet`
              }
            </div>
          )}
        </div>
