...
export default function Home() {
  ...
  const manulBet = async () => {
    ...
    const generateEntropyInBackground = async (historyItemId, winAmount, actualMultiplier, currentZetaEnabled, currentAddress, currentIsConnected) => {
      try {
        ...
        logGame({
          ...
          if (txHash) {
            console.log('✅ Wheel game logged to Creditcoin:', getExplorerUrl(txHash));
            // Update game history with Creditcoin transaction hash
            setGameHistory(prev => prev.map(item =>
              item.id === historyItemId
                ? { ...item, creditcoinTxHash: txHash }
                : item
            ));
          }
        }).catch(error => {
          console.warn('⚠️ Failed to log Wheel game to Creditcoin:', error);
        });
    ...
    // Check Redux balance (balance is already in CTC)
    const currentBalance = parseFloat(userBalance || '0');

    if (currentBalance < betAmount) {
      alert(`Insufficient balance. You have ${currentBalance.toFixed(5)} CTC but need ${betAmount} CTC`);
      return;
    }
      ...
      window.wheelBetCallback = async (landedMultiplier) => {
        ...
        setTimeout(async () => {
          ...
          if (actualMultiplier > 0) {
            notification.success(`Congratulations! ${betAmount} CTC × ${actualMultiplier.toFixed(2)} = ${winAmount.toFixed(5)} CTC won!`);
  ...
  const autoBet = async ({
    ...
    for (let i = 0; i < numberOfBets; i++) {
      ...
      if (currentBalance < currentBet) {
        alert(`Insufficient balance for bet ${i + 1}. Need ${currentBet.toFixed(5)} CTC but have ${currentBalance.toFixed(5)} CTC`);
        break;
      }
      ...
      if (actualMultiplier > 0) {
        notification.success(`Congratulations! ${currentBet} CTC × ${actualMultiplier.toFixed(2)} = ${winAmount.toFixed(8)} CTC won!`);
      }
