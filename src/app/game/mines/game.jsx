...
const Game = ({ betSettings = {}, onGameStatusChange, onGameComplete }) => {
  ...
  const defaultSettings = {
    betAmount: 0.001, // Default to 0.001 CTC
    mines: 5,
    isAutoBetting: false,
    tilesToReveal: 5,
  };
  ...
  useEffect(() => {
    ...
    if (Object.keys(betSettings).length > 0) {
      ...
      const startGameWithBet = async () => {
        ...
        // Check Redux balance CTC)
        const currentBalance = parseFloat(userBalance || '0');
        
        if (currentBalance < parseFloat(settings.betAmount)) {
          toast.error(`Insufficient balance. You have ${currentBalance.toFixed(5)} CTC but need ${parseFloat(settings.betAmount)} CTC`);
          return;
        }
        ...
        try {
          ...
          toast.success(`Bet placed! ${parseFloat(settings.betAmount).toFixed(5)} CTC deducted from balance`);
          ...
          if (settings.isAutoBetting && settings.aiAssist) {
            ...
            toast.info(`Bet placed: ${parseFloat(settings.betAmount).toFixed(5)} CTC, ${settings.mines} mines`);
          }
  ...
  const cashout = () => {
    ...
    try {
      ...
      // The actual payout was already handled in the initial bet transaction
              toast.success(`Cashed out: ${payout.toFixed(5)} CTC (${multiplier.toFixed(2)}x)`);
      playSound('cashout');
        ...
        {hasPlacedBet && isPlaying && !gameOver && !gameWon && (
          <div className="flex gap-3">
            ...
            >
              ...
              <span>CASH OUT ({calculatePayout()} CTC)</span>
            </button>
          </div>
        )}
        ...
        {gameWon && (
          <div className="text-center py-3 bg-gradient-to-r from-green-600 to-emerald-600 rounded-lg text-white font-bold">
            ...
            <div className="mt-2 text-sm opacity-90">
              Winnings: {calculatePayout()} CTC ({multiplier.toFixed(2)}x)
            </div>
          </div>
        )}
