...
export default function Mines() {
  ...
  const handleFormSubmit = async (formData) => {
    try {
      ...
      console.log('🔗 Network: CreditCoin Testnet | Token: CTC | Protocol: Pyth Entropy');
      ...
      console.log(`🎮 Game Config: ${formData.mines || 3} mines | ${formData.betAmount || '0.01'} CTC bet`);
  ...
  const handleGameComplete = async (result) => {
    ...
    try {
      ...
      logGame({
        ...
        if (txHash) {
          console.log('✅ Mines game logged to Creditcoin:', getExplorerUrl(txHash));
        ...
        console.warn('⚠️ Failed to log Mines game to Creditcoin:', error);
      });
    ...
    const newHistoryItem = {
      id: Date.now(),
      mines: result.mines || 0,
      bet: `${result.betAmount || '0.00000'} CTC`,
      outcome: result.won ? 'win' : 'loss',
      payout: result.won ? `${result.payout || '0.00000'} CTC` : '0.00000 CTC',
